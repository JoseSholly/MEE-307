...
def solve():
    ...
    if (reynold_number < 2000):
        ...
        Rey_ans= ttk.Label(root, text= '{:.2f}'.format(reynold_number),style= 'label.TLabel').grid(column=1, row=10, sticky=tk.W, ipadx=50)
        head_loss= ttk.Label(root, text= '{:.2f} '.format(headloss),style= 'label.TLabel').grid(column=1, row=11, sticky=tk.W, ipadx=50)
...
def main_body():
    ...
    root= tk.Tk()
    root.title("Fluid Mechanics (Horizontal)")
